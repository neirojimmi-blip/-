"""Хук: короткий, лицо открыто, прописные Bad Script без утолщения.
...
fscr = fit(F + "BadScript-Regular.ttf", max(SCRIPT, key=len), int(W * 0.66))
...
LH_C, LH_S = int(fcap.size * 1.36), int(fscr.size * 1.12)
...
print(f"кадров {N} ({DUR} c) | капс {fcap.size}px | Bad Script {fscr.size}px без утолщения")

"""Плашка кодового слова и финальная карточка для рилса про доступ к рабочей среде.
...
def cta(outdir="n3/cta", dur=2.60, cy=1150):
...
# ---------- финальная карточка ----------
def final(outdir="n3/fin", dur=4.50):
    os.makedirs(outdir, exist_ok=True)
    n = int(dur * FPS)
    ctx = ["ОТДАЮ БЕСПЛАТНО", "ПРИШЛЮ В ДИРЕКТ"]
